...
export class WalletController {
  ...
  @UseGuards(AuthGuard)
  @Post('credit')
  async fundWallet(@Request() req, @GetUser() user: any) {
    ...
    const card = await this.authService.cardDecryption(wallet.user.card);
    const cardCvv = await this.authService.cardDecryption(wallet.user.cardCvv);
    ...
    const payloadObj = {
      ...
      enckey:this.configService.get('FLW_ENC_KEY'),
  ...
  @UseGuards(AuthGuard)
  @Get('user/wallet')
  async getWallet(@Request() req, @GetUser() user: any) {
    console.log(user);
    const wallet = await this.walletService.checkIfWalletExists({
      where: { user: { id: user.userId } },
    });
  ...
  @UseGuards(AuthGuard)
  @Post('debit')
  async withdrawFromWallet(@Request() req, @GetUser() user: any) {
    const ref = `funded-${Math.floor(Math.random() * 10000000 + 1)}-${
      user.userId
    }`;
